...
class db_controler:
    ...
    def __init__(self):
        ...
        authority integer,token text,ltime real, PRIMARY KEY(account))''')
    ...
    def user_search(self, user_ac):
        info = (user_ac,)
        self.cur.execute("SELECT ltime FROM user WHERE account=?", info)
        self.database.commit()
        return self.cur.fetchone()
    
    def user_login(self, user_ac, user_pw):
        info = (user_ac, user_pw)
        self.cur.execute("SELECT * FROM user WHERE account=? AND password=?", info)
        self.database.commit()
        return self.cur.fetchone()

    def user_search_token(self, token):
        info = (token,)
        self.cur.execute("SELECT token FROM user WHERE token=?", info)
        self.database.commit()
        return self.cur.fetchone()

    def user_search_time(self, token):
        info = (token,)
        self.cur.execute("SELECT ltime FROM user WHERE token=?", info)
        self.database.commit()
        return self.cur.fetchone()
    ...
    def user_update_bytoken(self, token, ltime):
        info = (token, ltime, token)
        
        try:
            self.cur.execute("UPDATE user SET token=?, ltime=? WHERE token=?",info)
            self.database.commit()
        except sqlite3.ProgrammingError:
            return "update error"
